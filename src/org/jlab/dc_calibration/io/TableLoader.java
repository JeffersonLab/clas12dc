...
public class TableLoader
{
	...
	public static synchronized void Fill()
	{
			...
			{ // loop over slys
				...
				for (int ibfield = 0; ibfield < 6; ibfield++)
				{
					...
					for (int icosalpha = 0; icosalpha < 6; icosalpha++)
					{
						...
						double cos30minusalpha = Math.cos(Math.toRadians(30.))
								+ (double) (icosalpha) * (1. - Math.cos(Math.toRadians(30.))) / 5.;
		 ...
		 * for(int icosalpha =0; icosalpha<6; icosalpha++) {
		 * 
		 * for (int tb = 0; tb< maxBinIdxT[s][r][ibfield][icosalpha]; tb++) System.out.println("s "+(s+1)+" sl "+(r+1)+" ibfield "+ibfield+" icosalpha "+icosalpha+"tbin "+ tb +
		 * " val "+ DISTFROMTIME[s][r][ibfield][icosalpha][tb]);
		 * 
		 * } } } }
		 */
		T2DLOADED = true;
	 ...
	 */
	public static synchronized double calc_Time(
			double x,
			double dmax,
			double tmax,
			double alpha,
			...
			int r)
	{
		...
		// first, calculate n
		double n = (1. + (CCDBConstants.getDELTANM()[s][r] - 1.)
				* Math.pow(FracDmaxAtMinVel, CCDBConstants.getDELTANM()[s][r]))
				/ (1. - Math.pow(FracDmaxAtMinVel, CCDBConstants.getDELTANM()[s][r]));
		// now, calculate m
		...
		double balpha = (tmax - dmaxalpha / CCDBConstants.getV0()[s][r] - a * Math.pow(cos30minusalpha, n))
		...
		double deltatime_bfield = CCDBConstants.getDELT_BFIELD_COEFFICIENT()[s][r] * Math.pow(bfield, 2) * tmax
				* (CCDBConstants.getDELTATIME_BFIELD_PAR1()[s][r] * xhatalpha
						+ CCDBConstants.getDELTATIME_BFIELD_PAR2()[s][r] * Math.pow(xhatalpha, 2) +
						CCDBConstants.getDELTATIME_BFIELD_PAR3()[s][r] * Math.pow(xhatalpha, 3)
						+ CCDBConstants.getDELTATIME_BFIELD_PAR4()[s][r] * Math.pow(xhatalpha, 4));
