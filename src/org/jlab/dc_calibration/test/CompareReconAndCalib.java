...
public class CompareReconAndCalib
{
	...
	public static void main(String[] args)
	{
		...
		GStyle.getGraphErrorsAttributes()
				.setTitle("Time (ns) vs Distance (cm) for S " + (secIndex + 1) + " SL " + (slIndex + 1)
						+ " Reconstruction (dotted line) vs Calibration (solid line) for " + min_ang_degree
						+ " degree and B = 0 T to 1.5 T");
		// GStyle.getGraphErrorsAttributes().setTitle("Time (ns) vs Distance (cm) for S " + (secIndex + 1) + " SL " + (slIndex + 1) + " Reconstruction (dotted line) vs Calibration
		// (solid line) for 10 degree and B= 0.0 , 0.5, 1.0, 1.5 T");
		...
		defPars.LoadCCDB();
		double maxRange = 2.0 * wpdist[slIndex];//0.8;
		double minRange = 0.0;
		...
		for (double angDegree = min_ang_degree; angDegree <= max_ang_degree; angDegree += 5)
		{
			CalibTimeFunction myFnc = new CalibTimeFunction("myFnc", minRange,
					maxRange * Math.cos(Math.toRadians(30 - angDegree)));
